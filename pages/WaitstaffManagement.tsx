            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="flex bg-gray-50 p-1 rounded-2xl border border-gray-100">
                {(['GARCOM', 'GERENTE'] as const).map(role => (
                   ...
                   >
                     {role === 'GERENTE' ? <Crown size={14}/> : <Users size={14}/>}
                     {role}
                   </button>
                ))}
              </div>
              ...
              staff.map(member => (
                <div key={member.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center border shadow-sm ${member.role === 'GERENTE' ? 'bg-orange-50 text-orange-500 border-orange-100' : 'bg-white text-primary border-gray-200'}`}>
                      {member.role === 'GERENTE' ? <Crown size={18} /> : <Users size={18} />}
                    </div>
                    <div>
